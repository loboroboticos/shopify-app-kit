...
| `.claude/settings.json` | Pins the kit marketplace and plugin, registers the four vendored guards and the bootstrap hook. | `.claude/hooks/kit/*.sh` |
...
| `.claude/rules/prisma.md` | Path-scoped rule seed: migrations own RLS and roles, no `db push` or `migrate reset` (the `guard-migrations` hook blocks the destructive forms), the drift check. | `database.*`, `paths.prisma` |
