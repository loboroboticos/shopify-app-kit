...
4. **Check vendored hooks.** For every `${CLAUDE_PROJECT_DIR}/.claude/hooks/kit/*.sh`, line 2 must read
   ...
   file (`lib.sh` and `guard-shopify-cli.sh` are expected since v0.1.0; `guard-protected-branch.sh` and
   `guard-package-manager.sh` since v0.3.0). Compare `kit.version` with the plugin's own
